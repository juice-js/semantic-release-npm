...
export default async function (
  ...
  try {
    ...
    let updatePackages = localPackages || [];

    const pkg = await getPkg(pluginConfig, context);
    const registry = getRegistry(pkg, context);

    // convert updatePackages to peerDependenciesList with version
    if(typeof(updatePackages) === "string") {
      updatePackages = [updatePackages];
    }
    logger.debug("updatePackages: %o", updatePackages);

    if(Array.isArray(updatePackages)) {
      const peerDependenciesList = [];
      for(let i = 0; i < updatePackages.length; i++) {
        peerDependenciesList.push(`${updatePackages[i]}@${version}`);
      }
